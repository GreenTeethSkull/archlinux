...
while true
    ...
    if test $battery_level -ge 91 -a $is_charging = 'Charging'
        #notify-send "Battery is charge!" "Level: $battery_level%"
        zenity --info --title "Battery" --text "Battery is charge : $battery_level %"    
    else if test $battery_level -le 21 -a $is_charging = 'Discharging'
        #notify-send "Battery is lower!" "Level: $battery_level%"
        zenity --info --title "Battery" --text "Battery is lower : $battery_level %"     
    end
